...
cdef class KnnQuery(object):
    ...
    def topk(self, FloatMatrix items, FloatMatrix m, int k, FloatMatrix item_norms=None,
...
cdef class FloatMatrix(object):
    ...
    def assign_rows(self, rowids, FloatMatrix other):
        cdef IntVector rows
...
cdef class HalfMatrix(object):
    cdef CppMatrix[half] * c_matrix
    ...
    def __cinit__(self, X):
        ...
        if cai:
            shape = cai["shape"]
            data = cai["data"][0]
            self.c_matrix = new CppMatrix[half](shape[0], shape[1], <half*>data, False)
        else:
            # otherwise assume we're a buffer on host
            c_X = X
            self.c_matrix = new CppMatrix[half](X.shape[0], X.shape[1], &c_X[0, 0], True)

    @classmethod
    def zeros(cls, rows, cols):
        ret = HalfMatrix(None)
        ret.c_matrix = new CppMatrix[half](rows, cols, NULL, True)
        return ret
    ...
    def __getitem__(self, idx):
        ...
        if isinstance(idx, slice):
            ...
            start = idx.start if idx.start is not None else 0
            stop = idx.stop if idx.stop is not None else self.c_matrix.rows
            ret.c_matrix = new CppMatrix[half](dereference(self.c_matrix), start, stop)

        elif isinstance(idx, int):
            i = idx
            ret.c_matrix = new CppMatrix[half](dereference(self.c_matrix), i)
            ...
            ids = IntVector(idx)
            ret.c_matrix = new CppMatrix[half](dereference(self.c_matrix), dereference(ids.c_vector))

        return ret

    def assign_rows(self, rowids, HalfMatrix other):
        cdef IntVector rows
        rows = IntVector(np.array(rowids).astype("int32"))
        self.c_matrix.assign_rows(dereference(rows.c_vector), dereference(other.c_matrix))
...
cdef class LeastSquaresSolver(object):
    cdef CppLeastSquaresSolver[float] * c_solver
